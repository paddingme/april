...
$(document).ready(function() {
    ...
    if ($('html').hasClass('audio')) {
        soundtrack.addEventListener('ended', function(event) {
            animated = false;
            soundtrack.currentTime = 0;
            soundtrack.play();
        }, false);
        soundtrack.addEventListener('timeupdate', function(event) {
            if (!animated && soundtrack.currentTime > sync) {
                animated = true;
                monster
                    .css({marginLeft: "-100%"})
                    .show()
                    .animate({marginLeft: "200%"}, 5000, function() {
                        monster.hide();
                    });
                ;
            }
    ...
    if ($('html').hasClass('csstransitions')) {
        cta.find('a').live('click', function(event) {
            if (!cta.hasClass('display')) {
